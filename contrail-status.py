...
CONTRAIL_SERVICES = {'vrouter' : {'nodemgr' : 'contrail-vrouter-nodemgr',
                     ...
                     'config' : {'nodemgr' : 'contrail-config-nodemgr',
                                 ...
                                 'svcmonitor' : 'contrail-svc-monitor',
                                 'devicemgr' : 'contrail-device-manager'},
                     'config_database' : ['cassandra',
                                          'zookeeper',
                                          'rabbitmq'],
...
def container_status(pod,svc_name):
  for container in client.containers.list(filters={"label":"net.juniper.contrail.pod=" + pod}):
    if 'net.juniper.contrail.service' in container.labels:
      if container.labels['net.juniper.contrail.service'] == svc_name:
...
#                      help="ca-certificate file to use for HTTP requests to services")
    ...
    vrouter =  bool(client.containers.list(filters={"label":"net.juniper.contrail.pod=vrouter"}))
    control =  bool(client.containers.list(filters={"label":"net.juniper.contrail.pod=control"}))
    config =  bool(client.containers.list(filters={"label":"net.juniper.contrail.pod=config"}))
    config_database =  bool(client.containers.list(filters={"label":"net.juniper.contrail.pod=config_database"}))
    ...
    if config_database:
        contrail_service_status('config_database', options)
        print ""
    ...
    if analytics_database:
        contrail_service_status('analytics_database', options)
        print ""
